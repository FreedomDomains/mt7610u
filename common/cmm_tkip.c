	...
	IN	UINT			Len)
{
	...
	if(memcmp(pAd->PrivateInfo.Rx.MIC, OldMic, 8) != 0)
	{
...
/*
	...
	Arguments:
		pAd		Pointer	to our adapter
		struct sk_buff *	Pointer to Ndis Packet for MIC calculation
	...
	IN 		UINT16			*DataByteCnt)
{
	...
	memmove(TrailMIC, plaintext_ptr + plaintext_len, LEN_TKIP_MIC);
	...
	if (memcmp(MIC, TrailMIC, LEN_TKIP_MIC) != 0)
	{
...
/*	ARC4_CTX_STRUC ARC4_CTX;*/
	...
	memmove(ekey, iv, LEN_KEY_DESC_IV);
	memmove(&ekey[LEN_KEY_DESC_IV], key, LEN_PTK_KEK);
