...
#define os_free_mem(_pAd, _pVA)					os_free_mem(_pAd, _pVA)
...
#define PACKET_CB_ASSIGN(_p, _offset)	\
	(*((u8 *)_p + RTPktOffsetCB + _offset))

#define PACKET_CB(_p, _offset)	\
	(*((u8 *)_p + RTPktOffsetCB + CB_OFF + _offset))
...
#define RTMP_SET_PACKET_PROTOCOL(_p, _protocol) {\
	(PACKET_CB(_p, 23) = (u8)((_protocol) & 0x00ff)); \
	(PACKET_CB(_p, 24) = (u8)(((_protocol) & 0xff00) >> 8)); \
}
